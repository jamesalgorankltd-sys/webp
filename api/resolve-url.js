...
async function extractCandidatesWithHeadlessBrowser(pageUrl) {
  ...
  try {
    ...
    return (data || [])
      ...
      .sort((a,b) => ((b.score || 0) + matchPageScore(b.url, pageUrl, b.hint)) - ((a.score || 0) + matchPageScore(a.url, pageUrl, a.hint)))
      .map(x => x.url);
  } finally {
...
export default async function handler(req, res) {
  try {
    ...
    candidates = rankForOriginalPage(filterUsefulImageCandidates(candidates, url), url);
