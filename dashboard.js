...
const DEFAULT_SETTINGS = {
  ...
  backendUrl: 'https://backend-tdnn.vercel.app/api/convert',
  openRouterKey: '',
...
async function extractImageCandidatesFromPageUrl(pageUrl){
  ...
  const known = directFromKnownPageUrl(pageUrl); if(known) add(known, 3000); // low priority because guessed URLs can 404
  return uniqueList(out
    .filter(x=>!/(logo|icon|avatar|sprite|placeholder|tracking|pixel|adsbygoogle)/i.test(x.url))
...
async function resolveInputCandidatesForUpload(input){
  ...
  const candidates=[];
  try{ const backend = await tryBackendResolve(url); if(backend) candidates.push(backend); }catch(e){}
  ...
  const known = directFromKnownPageUrl(url); if(known) candidates.push(known);
  ...
  try{
    ...
    for(let i=0;i<candidates.length;i++){
      ...
      try{
        ...
        const up = await uploadToCloudinary(uploadInput, 'single-image');
        progress(100); setStatus('Done. WebP source link ready.','Best available image converted. Quality setting: WebP q_auto:best, no forced blur/downscale.'); setActivity('Complete • WebP CDN source link is ready', 'done');
    ...
    if(/404|not found/i.test(msg)) hint = 'Page ne broken image URL diya. Is fixed version mein alternate candidates bhi try hoti hain; agar phir bhi fail ho to direct image address ya drag/drop use karein.';
